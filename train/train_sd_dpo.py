...
from utils.train_utils import log_validation, ModelEMA
...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Drop the reference model.",
    )
    parser.add_argument(
        "--do_clip",
        action="store_true",
        default=False,
        help="Wether or not to clip the ratio.",
    )
    ...
    parser.add_argument("--eta_dpo", type=float, default=0.001, help="The beta SFT temperature controlling strength of KL penalty")
    parser.add_argument("--linear_dpo", action='store_true', default=False, help="The beta SFT temperature controlling strength of KL penalty")
    parser.add_argument("--use_ema_ref", action='store_true', default=False, help="Use EMA reference model")
    parser.add_argument("--decay_ema", type=float, default=0.9999, help="Decay rate for EMA reference model")
    parser.add_argument("--valid_ema", action='store_true', default=False, help="Validate EMA reference model")
    parser.add_argument(
...
def main():
    ...
    if args.use_ema_ref:
        ref_unet = ModelEMA(unet, decay=args.decay_ema)
    ...
    if args.train_method == 'dpo' and not args.use_ema_ref: ref_unet.requires_grad_(False)
    ...
    if args.gradient_checkpointing: #  (args.sdxl and ('turbo' not in args.pretrained_model_name_or_path) ):
    ...
    # test prompts
    with open('data/pickapicv2_test500.txt', 'r') as f:
    ...
    if args.train_method == 'dpo':
        ...
        if args.choice_model:
            ...
            if args.choice_model == 'hps':
                ...
                from rewards.pick_score import PickScoreScorer
                selector = PickScoreScorer(accelerator.device)
    ...
    if 'pickapic' in args.dataset_name.lower():
        ...
        if args.max_train_samples is not None:
            dataset[args.split] = dataset[args.split].shuffle(seed=42).select(range(args.max_train_samples))
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                if args.train_method == 'sft': # SFT, casting for F.mse_loss
                    ...
                    raw_model_loss_w = model_losses_w.mean() 
                    raw_model_loss_l = model_losses_l.mean() 

                    model_diff = model_losses_w - model_losses_l # These are both LBS (as is t)
                    raw_model_diff = model_diff.mean()
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.validation_steps == 0:
                    ...
                    if args.valid_ema:
                        logger.info("Running ema validation...")
                        if args.sdxl:
                            # Serialize pipeline.
                            pipeline = StableDiffusionXLPipeline.from_pretrained(
                                args.pretrained_model_name_or_path, unet=accelerator.unwrap_model(ref_unet.ema_model), 
                                vae=vae, revision=args.revision, torch_dtype=weight_dtype
                            )
                        else:
                            pipeline = StableDiffusionPipeline.from_pretrained(
                                args.pretrained_model_name_or_path,
                                vae=vae,
                                unet=accelerator.unwrap_model(ref_unet.ema_model),
                                revision=args.revision,
                                torch_dtype=weight_dtype,
                            )
                        images = log_validation(
